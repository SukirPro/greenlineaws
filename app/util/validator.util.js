...
// const log = log4js.getLogger('Validator')
...
exports.validateBody = schema => (req, res, next) => {
    ...
    if (error) {
        ...
        const err = error.details.map(( item ) => {
            customError[item.path] = _.upperFirst(_.toLower(_.startCase(_.toLower(item.message)))).replace("id ","");
        })
        // log.error(err)
        return response.validationError(res, "The given data was invalid.", customError)
    } else next();
}
